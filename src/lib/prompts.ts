...
${contextLine ? `\n${contextLine}\n` : ""}
Rules:
- Set recognised to true only if an exercise movement is actively happening in the frame. If the person is standing still, sitting, out of frame, or no workout is visible, set recognised to false and leave cues empty.
...
- Positive: one short observation if something is notably good, max 6 words. Omit entirely if nothing stands out or if recognised is false.`;
  }
